...
function ifRule(element) {
    ...
    var condition = element.attribs.condition;
        ...
        endTag=tools.getIfendTag(element);
...
function textRule(element) {
    ...
    if(element.data.trim() !== "") {
        ...
        function getRuletoApply(element){
              var RuletoApply;
            if(element.data.indexOf('$')){
              return "${}"
            }else if (element.data.indexOf('#')) {
              return "#Html.RenderAction"
            }
        }

        switch(getRuletoApply(element)) {
