...
function ViewDataRule(element){
   var startTag ="",endTag = "",attributes;
       attributes = decoder.renderAttributes(element.attribs);
      return ['<%\/\*'+"<"+element.name+attributes+">\*\/%>",endTag];
}
...
module.exports.applyRule = function(element) {
   ...
   switch(condition) {
        ...
        case 'root':
           return ["",""];
        default: return DefaultRule(element);
