...
var EMPTY_ELEMENT = { startTag: '', endTag: '' };
...
function TestRule(element) {
  ...
  var condition = tools.ApplyRegularExpression(element.attribs.if);
...
function ViewDataRule(element) {
    var startTag = "";
    var endTag = "";
    startTag += decoder.renderViewdataAttributes(element.attribs);
...
function ContentRule(element) {
  ...
  return { startTag, endTag, inScript: true };
}
...
function CacheRule(element) {
  ...
  function VarRule(element) {
    var startTag = "",
      endTag = "";
    startTag += decoder.renderVarAttributes(element.attribs).trim();
  ...
  function IfRule(element) {
    var startTag = "<% if (:condition) { %>";
    var endTag;
    var condition = tools.ApplyRegularExpression(element.attribs.condition, true);
  ...
  function IfAttributeRule(element) {
    ...
    var startTagWhenNotElse = "<% if (:condition) { %>\n <:element :attributes >";
    var startTagWithElse = "<% } else if (:condition) { %>";

    var endTagWhenNotElse = "</:element> \n <% } %>";
    var endTagWhenElse = "<% } %>";
    ...
    if (element.name === 'else') {
      retValue = {
        startTag: startTagWithElse.replace(':condition', condition),
        endTag: endTagWhenElse
      };
    } else if (condition !== undefined) {
      retValue = {
        startTag: startTagWhenNotElse
          .replace(':condition', condition)
          .replace(":element", element.name)
          .replace(":attributes", decoder.renderAttributes(element.attribs)),
        endTag: endTagWhenNotElse.replace(":element", element.name)
      };
    }
  ...
  function ForeachRule(element) {
    var startTag = "<% :object.forEach(function(:index, :indexIndex) { %>",
      endTag = "<% }) %>",
      argumentsFor = element.attribs.each.split(" ");

    if (arguments.length > 0) {
      startTag = startTag.replace(':object', argumentsFor[3]).replace(/:index/g, argumentsFor[1]);
    }
  ...
  function RenderPartialRule(element) {
    ...
    return { startTag: renderTag.trim(), endTag: '' };
  }
  /**
  ...
  function DefaultRule(element) {
    var elementToReturn = EMPTY_ELEMENT;
    var attributes = decoder.renderAttributes(element.attribs);

    if (element.name) {
      if (element.name === 'link' || element.name === 'meta') {
        elementToReturn = {
          startTag: "<" + element.name + attributes + "/>",
          endTag: ""
        };
      } else if (element.name === '!doctype') {
        elementToReturn = {
  ...
  module.exports.applyRule = function (element) {
    ...
    case 'root':
      return _(EMPTY_ELEMENT).extend({isRoot: true});
    default:
