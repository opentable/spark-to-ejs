...
*/
// TODO: this does not capture expressions that contain quotes
var unescapeBlock = /"[^"]+"|(\!\{)([^\}^\"]+))(\})/g;
var escapeBlock = /"[^"]+"|(\$\{)([^\}^\"]+)(\})/g;
var expStringJoin = /([Ss]tring\.[jJ]oin\()([^\)]+)(\))/g;
var expStringFormat = /([Ss]tring\.[fF]ormat\()([^\)]+)(\))/g;

var expIsNullOrEmpty = /([Ss]tring\.IsNullOrEmpty\()(.+)(\))/g;
var expIsNullOrWhiteSpace = /([Ss]tring\.IsNullOrWhiteSpace\()(.+)(\))/g;
...
var CheckBlockCode = function (input, inScript, inQuotes) {
  ...
  input = input.replace(expStringJoin, function (match, left, center, right) {
    return 'String.Join(' + center + ')';
  });

  input =input.replace(expStringFormat, function (match, left, center, right) {
  ...
  input = input.replace(unescapeBlock, function (match, left, center, right) {
    ...
    return wrapWithBrackets(center, inScript, inQuotes, true);
  });
...
exports.ApplyRegularExpression = function (input, inScript, inQuotes) {
  ...
  var expStringJoin2 = /([\$|\!]\{[Ss]tring\.[jJ]oin\()([^\)]+)(\)\})/g;
  ...
  if (input.match(expBooleanBlock1)) {
    ...
    return CheckBlockCode(input, inScript, inQuotes);
  } else if (input.match(expStringJoin)) {
    return CheckBlockCode(input, inScript, inQuotes);
  } else if (input.match(expStringJoin2)) {
    return CheckBlockCode(input, inScript, inQuotes);
