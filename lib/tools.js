...
function wrapWithBrackets(input, inScript) {
  return inScript ? input : '<%= ' + input + ' %>';
}
...
* @return
*/
var CheckBooleanBlock = function (input, inScript) {
  var expValue;
  expValue = /([a-zA-z-_]{1,}\=\"checked\?\{)(.+)(\}\")/g;
  input = input.replace(expValue, function (match, left, center, right) {
    left = left.substring(0, left.length - 2);
    return wrapWithBrackets(center + ' ? \' checked\' : \'\'');
  });

  expValue = /([a-zA-z-_]{1,}\=\"selected\?\{)(.+)(\}\")/g;
  input = input.replace(expValue, function (match, left, center, right) {
    left = left.substring(0, left.length - 2);
    return wrapWithBrackets(center + ' ? \' selected\' : \'\'');
  });

  expValue = /([a-zA-z-_]{1,}\=\"disabled\?\{)(.+)(\}\")/g;
  input = input.replace(expValue, function (match, left, center, right) {
    left = left.substring(0, left.length - 2);
    return wrapWithBrackets(center + ' ? \' disabled\' : \'\'');
  });

  expValue = /([a-zA-z-_]{1,}\?\{)([^\}]+)(\})/g;
  input = input.replace(expValue, function (match, left, center, right) {
    left = left.substring(0, left.length - 2);
    return wrapWithBrackets(center + ' ? \'' + left + '\' : \'\'');
  });
...
*/
var CheckBlockCode = function (input, inScript) {
  ...
  input = input.replace(expValue, function (match, left, center, right) {
    return wrapWithBrackets(center, inScript);
  });
...
*/
var CheckRenderActionBlock = function (input, inScript) {
  var expValue;
  expValue = /(\#Html\.RenderAction\(\")(\w+)("\,.+)/g;
  input = input.replace(expValue, function (match, left, center, right) {
    return wrapWithBrackets('Html.RenderAction(' + center + ')', inScript);
  });
  return input;
};

exports.ApplyRegularExpression = function (input, inScript) {
  ...
  if (input.match(expStringFormat1)) {
    return CheckStringFormatBlock(input, inScript);
    ...
    return CheckBooleanBlock(input, inScript);
    ...
    return CheckRenderActionBlock(input, inScript);
  } else if (input.match(expIsNullOrEmpty)) {
    return CheckBlockCode(input, inScript);
