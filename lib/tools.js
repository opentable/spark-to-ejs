...
var expString = /([Ss]tring)\.(?:[jJ]oin|[fF]ormat|[iI]sNull)/g;
...
var CheckBlockCode = function (input, inScript, inQuotes) {
  input = input.replace(unescapeBlock, function (match, params) {
    return wrapWithBrackets('(' + params + ')', inScript, inQuotes, true);
  });

  input = input.replace(escapeBlock, function (match, params) {
    return wrapWithBrackets('(' + params + ')', inScript, inQuotes);
  });
...
exports.ApplyRegularExpression = function (input, inScript, inQuotes) {
  ...
  input = input.replace(expString, function(match, params) {
    return match.replace('string', 'String');
  });
